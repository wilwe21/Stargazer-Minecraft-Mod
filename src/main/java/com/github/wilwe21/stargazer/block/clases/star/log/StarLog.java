package com.github.wilwe21.stargazer.block.clases.star.log;
...
public class StarLog extends FacingBlock {
    @Override
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (stack.streamTags().toList().contains(ItemTags.AXES)) {
            BlockState newState = StarBlocks.STRIPPED_STAR_LOG.getDefaultState().with(Properties.AXIS, state.get(Properties.AXIS));
    ...
    public StarLog(Settings settings) {
        super(settings);
